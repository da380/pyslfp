...
def main():
    ...
    eval_op = sl.ice_sheet_averaging_operator(model_space, exact_fp)

    # Automatically build the parameterized Bayesian inversion problem
    param_inv = inverse_problem.parameterized_inversion(
        basis_op,
        dense=True,
        formalism="model_space",
        parameter_prior=inf.GaussianMeasure.from_standard_deviation(
            basis_op.domain, args.prior_std_mm / to_mm
        ),
        parallel=True,
        n_jobs=10,
    )
    ...
    if args.parameterized_truth:
        # Generate truth from the 25 regional parameters using the autogenerated prior
        param_truth = param_inv.model_prior_measure.sample()
    ...
    # param_inv was already built in Step 3!
    param_posterior = param_inv.model_posterior_measure(data, inf.CholeskySolver())
    ...
    if args.plot_maps:
        ...
        # Using the updated forward_problem attribute!
        param_data_expectation = param_inv.forward_problem.forward_operator(
            param_posterior.expectation
        )
    ...
    if args.plot_corners:
        print("\nExtracting and densifying covariances ONLY for corner plot regions...")
        ...
        # Create a scaling operator for the new 6D space
        to_mm_op = sub_sampler.codomain.identity_operator() * to_mm

        # Chain them: First extract the 6 dimensions, then scale them to mm
        extract_and_scale_op = to_mm_op @ sub_sampler

        # Map the posteriors to this smaller space and THEN densify the covariance
        param_sub_posterior = param_posterior.affine_mapping(
            operator=extract_and_scale_op
        ).with_dense_covariance(parallel=True, n_jobs=10)

        full_sub_posterior = full_posterior_3d.affine_mapping(
            operator=extract_and_scale_op
        ).with_dense_covariance(parallel=True, n_jobs=10)
