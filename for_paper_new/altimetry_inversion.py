...
and vertically averaged ocean density changes (reported as an effective steric
sea level), while strictly enforcing ocean mass conservation. Features spatial
covariance mapping to visualize the physical constraints, and an optional
push-forward onto the 2D split of GMSL change into barystatic and
ocean-dynamic contributions.
"""
...
def main():
    ...
    # ---------- 4b. GMSL SPLIT: BARYSTATIC vs OCEAN-DYNAMIC ----------
    if args.plot_gmsl_split:
        ...
        split_op = inf.ColumnLinearOperator([bary_op, dyn_op]) * mm_scale
        ...
        dyn_resid_mm = dyn_op(model)[0] * mm_scale
        ...
        with open(metrics_file, "a") as f_metrics:
            ...
            f_metrics.write(
                f"Bary-Dyn correlation:    prior {correlation(prior_cov_mat):+.4f} | "
        ...
        inf.plot_corner_distributions(
            ...
            labels=["Barystatic GMSL (mm)", "Ocean-dynamic GMSL (mm)"],
            title="",
            fill_density=False,
        )
