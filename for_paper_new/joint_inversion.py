...
topography, and vertically averaged ocean density changes (reported as an
effective steric sea level).

It runs Head-to-Head Altimetry-only and Joint (Alt + GRACE) inversions
on the exact same physical scenario to demonstrate the added value of
gravimetry, including a push-forward onto the 2D split of GMSL change into
barystatic and ocean-dynamic contributions.
"""
...
def parse_arguments():
    ...
    parser.add_argument(
        "--plot-gmsl-split",
        action="store_true",
        help="Push forward onto the 2D GMSL split (barystatic vs ocean-dynamic).",
    )
    parser.add_argument(
        "--std-samples",
        type=int,
...
def main():
    ...
    # ---------- 5b. GMSL SPLIT: BARYSTATIC vs OCEAN-DYNAMIC ----------
    if args.plot_gmsl_split:
        print("\nPushing forward onto the 2D GMSL split (barystatic, ocean-dynamic)...")
        bary_op, dyn_op, dyn_direct_op = utils.gmsl_split_operators(
            exact_phys["state"], exact_phys["load_space"], exact_phys["continuous_sl"]
        )
        split_op = inf.ColumnLinearOperator([bary_op, dyn_op]) * scale_mm
        ...
        with open(metrics_file, "a") as f_metrics:
            f_metrics.write(
                "\n\nGMSL Split: Barystatic vs Ocean-Dynamic (dyn. topography + steric)\n"
            )
            f_metrics.write("=" * 105 + "\n")
            f_metrics.write(
                f"{'Metric':<22} | {'Prior':<12} | {'Alt-Only':<12} | {'Joint':<12}\n"
            )
            f_metrics.write("-" * 105 + "\n")
            f_metrics.write(
            ...
            f_metrics.write(
                f"{'Bary-Dyn correlation':<22} | {correlation(prior_cov_mat):<+12.4f} | "
                f"{correlation(alt_cov_mat):<+12.4f} | "
                f"{correlation(joint_cov_mat):<+12.4f}\n"
            )
            f_metrics.write(
                f"Mass-conservation check: dyn residual = {dyn_resid_mm:.6f} mm | "
                f"direct avg = {dyn_direct_mm:.6f} mm | "
                f"diff = {dyn_resid_mm - dyn_direct_mm:.3e} mm\n"
            )
            f_metrics.write("-" * 105 + "\n")
            f_metrics.write(
                f"{'Component':<22} | {'Prior Var':<12} | {'Alt Var':<12} | "
                f"{'Alt Red%':<10} | {'Joint Var':<12} | {'Joint Red%'}\n"
            )
            f_metrics.write("-" * 105 + "\n")
            for i, name in enumerate(["Barystatic", "Ocean-dynamic"]):
        ...
        split_labels = ["Barystatic GMSL (mm)", "Ocean-dynamic GMSL (mm)"]
    ...
    if args.plot_maps:
        ...
        steric_scale = utils.effective_steric_scale(state)
    ...
    if args.plot_regions:
        ...
        steric_scale = utils.effective_steric_scale(exact_phys["state"])
