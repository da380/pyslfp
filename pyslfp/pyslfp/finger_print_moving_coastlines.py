...
class FingerPrint(EarthModelParameters, LoveNumbers):
    ...
    def _return_ocean_function_from_sea_level(self, current_sea_level: SHGrid) -> SHGrid:
        """
        Returns an updated ocean function given an absolute sea level field.

        This mirrors _compute_ocean_function but takes an explicit sea level
        field rather than self.sea_level, allowing it to be called mid-iteration.

        Args:
            current_sea_level: The absolute sea level field (background + change).

        Returns:
            An SHGrid ocean function (1 over ocean, 0 over land).
        """
        ocean_data = np.where(
            self.water_density * current_sea_level.data
            - self.ice_density * self.ice_thickness.data
            > 0,
            1,
            0,
    ...
    def __call__(
        ...
        *,
        ...
        Args:
            ...
            moving_coastline: If True, update the ocean function at each iteration
                based on the current sea level field. Requires that sea_level and
                ice_thickness have been set. Defaults to False.
        ...
        if moving_coastline and not self.background_set:
            raise AttributeError(
                "sea_level and ice_thickness must be set before using moving_coastline=True."
            )

        self._solver_counter += 1

        # --- Initialise ocean function and ocean area ---
        # For moving coastlines these are updated each iteration;
        # for static coastlines they remain fixed at the background values.
        current_ocean_function = self.ocean_function
        current_ocean_area = self.ocean_area

        if moving_coastline:
            # --- Store the background water load once ---
            # This is subtracted at each iteration so that the net load reflects 
            # only the change from the background state (load = after - before).
            background_water_load = (
                self.water_density * current_ocean_function * self.sea_level
            )

        mean_sea_level_change = -self.integrate(direct_load) / (
            self.water_density * current_ocean_area
        )

        load = (
            direct_load
            + self.water_density * current_ocean_function * mean_sea_level_change
        )
        ...
        while err > rtol:
            ...
            # Elastic + rotational part of sea level change, without uniform shift.
            non_uniform_contribution = (-1 / g) * (g * displacement + gravity_potential_change)

            if moving_coastline:

                # --- Add the uniform shift using the current ocean area estimate ---
                # This is so that the flooding condition is evaluated with the best 
                # available sea level before the coastline is updated.
                uniform_contribution = mean_sea_level_change - (
                    self.integrate(current_ocean_function * non_uniform_contribution)
                    / current_ocean_area
                )
                sea_level_change = non_uniform_contribution.copy()
                sea_level_change.data += uniform_contribution

                # --- Update the ocean function using the corrected absolute sea level ---
                # Any residual inconsistency (because mean_sea_level_change
                # was computed with the old ocean area) is second-order and resolves
                # over subsequent iterations.
                absolute_sea_level = self.sea_level + sea_level_change
                current_ocean_function = self._return_ocean_function_from_sea_level(
                    absolute_sea_level
                )
                current_ocean_area = self.integrate(current_ocean_function)

                # --- Recompute the uniform shift with the updated ocean area
                # to enforce mass conservation over the new coastline ---
                mean_sea_level_change = -self.integrate(direct_load) / (
                    self.water_density * current_ocean_area
                )

                # --- Recompute the final sea level change with the updated
                # ocean area so the uniform shift is consistent with the new C ---
                uniform_contribution = mean_sea_level_change - (
                    self.integrate(current_ocean_function * non_uniform_contribution)
                    / current_ocean_area
                )
                ...
                # --- Form the net load as (total load after) - (total load before) ---
                # total load after  = direct + rho_w * C_new * SL_new
                # total load before = rho_w * C_0 * SL_0  (= background_water_load)
                # where SL_new = SL_0 + sea_level_change.
                absolute_sea_level = self.sea_level + sea_level_change
                load_new = (
                    direct_load
                    + self.water_density * current_ocean_function * absolute_sea_level
                    - background_water_load
                )

            else:
                # --- Static coastline: original behaviour ---
                uniform_contribution = mean_sea_level_change - self.ocean_average(non_uniform_contribution)
                sea_level_change = non_uniform_contribution.copy()
                sea_level_change.data += uniform_contribution
                load_new = (
                    direct_load
                    + self.water_density * current_ocean_function * sea_level_change
                )
